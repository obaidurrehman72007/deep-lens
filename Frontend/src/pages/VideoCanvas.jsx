...
const VideoCanvas = () => {
  ...
  // Helper to handle navigation without passing events
      ...
      <header className="h-14 border-b bg-white flex items-center justify-between px-4 z-20 shadow-sm">
        <div className="flex items-center gap-4">
          ...
          <div className="flex items-center gap-2">
            <div className={`p-1.5 rounded-lg ${isPublicView ? "bg-amber-100 text-amber-600" : "bg-indigo-100 text-indigo-600"}`}>
  ...
  <p className="text-slate-300 text-xs font-medium tracking-wide">
    Unsaved changes: <span className="text-red-400 font-bold uppercase ml-1">Sync Required</span>
  </p>
</div>
        ...
        <div className="flex items-center gap-3">
          ...
          >
            {showChat ? <PanelLeftClose size={18} /> : <PanelLeftOpen size={18} />}
            <span className="hidden sm:inline">{showChat ? "Hide Sidebar" : "Show Sidebar"}</span>
          </button>
        </div>
      </header>
      ...
      <div className="flex-1 flex overflow-hidden relative">
        ...
        {/* LOGS OVERLAY */}
        <div className={`absolute top-0 right-0 h-full w-80 bg-white border-l shadow-2xl z-30 transform transition-transform duration-300 ease-in-out ${showLogs ? "translate-x-0" : "translate-x-full"}`}>
          <div className="p-4 border-b flex items-center justify-between bg-slate-50">
            <div className="flex items-center gap-2 font-bold text-slate-800 text-sm">
              <Activity size={16} className="text-amber-500" />
              Canvas Activity
            </div>
            <button onClick={() => setShowLogs(false)} className="p-1 hover:bg-slate-200 rounded text-slate-500">
              <X size={18} />
            </button>
          </div>
          <div className="overflow-y-auto h-[calc(100%-60px)] p-4 space-y-4">
            {logs.length === 0 ? (
              <div className="text-center py-10 text-slate-400 text-xs italic">No activity logs found.</div>
            ) : (
              logs.map((log) => (
                <div key={log._id} className="border-l-2 border-indigo-100 pl-3 py-1">
                  <div className="flex items-center justify-between mb-1">
                    <span className="text-[10px] font-bold text-indigo-600 flex items-center gap-1">
                      <User size={10} /> {log.userEmail?.split("@")[0] || "User"}
                    </span>
        ...
        {/* LEFT PANEL */}
        <div className={`flex flex-col border-r bg-slate-50 transition-all duration-300 shadow-inner ${showChat ? "w-[450px]" : "w-0 -translate-x-full"}`}>
